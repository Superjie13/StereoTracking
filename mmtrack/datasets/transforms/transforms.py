...
@TRANSFORMS.register_module()
class TLBRCrop(BaseTransform):
    """Crop the image from top-left to bottom-right.
    ...
    Args:
        crop_size (List[int]): Expected crop range, top-left~bottom-right (x,y,x,y).
        clip_object_border (bool): Whether to clip the objects
            outside the border of the image. In some dataset list MOT17, the
    ...
    def __init__(self,
        ...
        super().__init__()
        assert len(crop_size) == 4, 'The expected crop_size is a tuple containing four ' \
                                    'intergers (top-left bottom-right, i.e., x, y, x, y)'

        self.crop_size = list(crop_size)
        self.clip_object_border = clip_object_border

    def _crop_img(self, results: dict, crop_size: List[int]):
        """Crop image.
        ...
        if results.get('img', None) is not None:
            img = mmcv.imcrop(results['img'], bboxes=np.array(crop_size))
            img_shape = img.shape[:2]
            results['img'] = img
            results['img_shape'] = img_shape
            results['pad_shape'] = img_shape

    def _crop_disp(self, results: dict, crop_size: List[int]):
        """Crop disparity map.

        Args:
            resutls (dict): Result dict contains the data to transform.
        """
        if results.get('disp_postp', None) is not None:
            disp_postp = mmcv.imcrop(results['disp_postp'], bboxes=np.array(crop_size))
            results['disp_postp'] = disp_postp

    # @autocast_box_type()
    def _crop_bboxes(self, results: dict, crop_size: List[int]):
        """Update bounding boxes according to the crop size.

        Args:
            results (dict): Result dict contains the data to transform.
        """
        if 'gt_bboxes' in results:
            offset_w = crop_size[0]
            offset_h = crop_size[1]
            # gt_bboxes has shape (num_gts, 4) in (tl_x, tl_y, br_x, br_y)
            gt_bboxes = results['gt_bboxes']
            gt_bboxes.translate_([-offset_w, -offset_h])
            if self.clip_object_border:
                gt_bboxes.clip_(results['img'].shape[:2])
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        Args:
            results (dict): Result dict contains the data to tranform.

        Returns:
            dict: Results with cropped image and disparity map.
        """
        ...
        if self.crop_size[2] < 0:
            crop_size[2] = img_width
        if self.crop_size[3] < 0:
            crop_size[3] = img_height
        assert crop_size[0] >=0 and \
               crop_size[1] >=0 and \
               crop_size[2] <= img_width and \
               crop_size[3] <= img_height, f'crop size out of the range of original image' \
                                           f'crop_size: {crop_size}, image_size: ' \
                                           f'0, 0, {img_width}, {img_height}-{self.crop_size}'

        # crop the image
        self._crop_img(results, crop_size)
        # crop the disparity map
        self._crop_disp(results, crop_size)
        # crop the bounding box
        self._crop_bboxes(results, crop_size)
        return results
