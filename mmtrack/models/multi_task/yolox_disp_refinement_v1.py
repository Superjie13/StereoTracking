...
class YOLOX_DISP_Refinement_V1(SingleStageDetector_DispCompletion):
    ...
    def loss(self, batch_inputs: Dict[str, Tensor],
        ...
        data_in = dict(
            ...
            disp_postp = batch_inputs['disp_cutout']
        )

        if self.train_bbox:
            x = self.extract_feat(data_in)
            x = list(x)
        ...
        if self.train_disp:
            ...
            assert batch_inputs.get('disp_postp', None) is not None
            assert batch_inputs.get('disp_mask', None) is not None
            gt_disp = batch_inputs.get('disp_postp')[:, 0: 1, :, :]
            pixels_weight = batch_inputs.get('disp_mask')
