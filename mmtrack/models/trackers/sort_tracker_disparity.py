...
class SORTTracker_Disparity(BaseTracker):
    ...
    def track(self,
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            if len(active_ids) > 0:
                ...
                track_scale = self.get('scales', active_ids)

                # cam motion compensation
                ...
                track_bboxes = track_bboxes + offset.to(track_bboxes)

                # depth aware matching
                new_track_boxes = scale_bbox(track_bboxes, track_scale)
                new_det_boxes = scale_bbox(bboxes, scales)
                ious = bbox_overlaps(new_track_boxes, new_det_boxes[active_dets])
