...
class OCSORTTracker_Disparity(SORTTracker):
    """Tracker for OC-SORT.
    ...
    def init_track(self, id, obj):
        ...
        self.tracks[id].obs = []
        self.tracks[id].obs_scales = []
        bbox_id = self.memo_items.index('bboxes')
        scale_id = self.memo_items.index('scales')
        self.tracks[id].obs.append(obj[bbox_id])
        self.tracks[id].obs_scales.append(obj[scale_id])
    ...
    def ocm_assign_ids(self,
        ...
        # compute distance
        ious = bbox_overlaps(scaled_track_bboxes + offset.to(scaled_det_bboxes), scaled_det_bboxes[:, :4])
        if weight_iou_with_det_scores:
            ious *= det_scores[np.newaxis,]
    ...
    def last_obs_scale(self, track):
        """extract the last associated observation's scale."""
        for scale in track.obs_scales[::-1]:
    ...
    def ocr_assign_ids(self,
        ...
        Returns:
            tuple(int): The assigning ids.
        """
        scaled_track_obs = scale_bbox(track_obs[:, :4], track_obs_scales)
        scaled_det_bboxes = scale_bbox(det_bboxes[:, :4], det_scales)
        # compute distance
        # ious2 = bbox_overlaps(track_obs[:, :4], det_bboxes[:, :4])
        ious = bbox_overlaps(scaled_track_obs + offset, scaled_det_bboxes)
        # if offset.sum() > 0:
        #     print(f'\noffset: {offset}')
        #     print('ocr_iou: ', ious, '\nocr_iou2: ', ious2)
        if weight_iou_with_det_scores:
    ...
    def track(self,
        ...
        if self.empty or bboxes.size(0) == 0:
            # 第0帧图像中置信度大于 init_track_thr 的目标被初始化为新的track
            ...
            valid_area = (bboxes[:, 2] - bboxes[:, 0]) * (bboxes[:, 3] - bboxes[:, 1])
            valid_area = valid_area > 100
            det_inds = det_inds & valid_area
            det_bboxes = bboxes[det_inds]
            det_labels = labels[det_inds]
            det_scores = scores[det_inds]
            det_scales = scales[det_inds]
            det_depth = depth[det_inds]
            ...
            match_track_inds, raw_match_det_inds = self.ocm_assign_ids(
                self.confirmed_ids, det_bboxes, det_scores, det_scales,
            ...
            match_det_scales = det_scales[valid]
            match_det_depth = det_depth[valid]
            ...
            unmatch_det_scales = det_scales[~valid]
            unmatch_det_depth = det_depth[~valid]
            ...
            (tentative_match_track_inds,
             tentative_match_det_inds) = self.ocm_assign_ids(
                 self.unconfirmed_ids, unmatch_det_bboxes, unmatch_det_scores, unmatch_det_scales,
                 self.weight_iou_with_det_scores, self.match_iou_thr)
            ...
            match_det_scales = torch.cat(
                (match_det_scales, unmatch_det_scales[valid]), dim=0)
            match_det_depth = torch.cat(
                (match_det_depth, unmatch_det_depth[valid]), dim=0)
            assert (match_det_ids > -1).all()
            ...
            unmatch_det_scales = unmatch_det_scales[~valid]
            unmatch_det_depth = unmatch_det_depth[~valid]
            ...
            if len(unmatched_track_inds) > 0:
                ...
                last_observations = []
                last_observations_scales = []
                for id in unmatched_track_inds:
                    last_box = self.last_obs(self.tracks[id.item()])
                    last_box_scale = self.last_obs_scale(self.tracks[id.item()])
                    if cam_motion_valuable:
                        offset = torch.tensor([cam_speed[0], cam_speed[1], cam_speed[0], cam_speed[1]]).to(last_box)
                        # print('offset: ', offset)
                    else:
                        offset = torch.tensor([0,0,0,0]).to(last_box)
                    last_observations.append(last_box)
                    last_observations_scales.append(last_box_scale)
                last_observations = torch.stack(last_observations)
                last_observations_scales = torch.stack(last_observations_scales)
                ...
                _, ocr_match_det_inds = self.ocr_assign_ids(
                    last_observations, last_observations_scales, unmatch_det_bboxes, unmatch_det_scores, unmatch_det_scales,
                    self.weight_iou_with_det_scores, self.match_iou_thr, offset)
                ...
                ocr_match_det_scales = unmatch_det_scales[valid]
                ocr_match_det_depth = unmatch_det_depth[valid]
                ...
                ocr_unmatch_det_scales = unmatch_det_scales[~valid]
                ocr_unmatch_det_depth = unmatch_det_depth[~valid]
                ...
                unmatch_det_scales = ocr_unmatch_det_scales
                unmatch_det_depth = ocr_unmatch_det_depth
                ...
                match_det_scales = torch.cat(
                    (match_det_scales, ocr_match_det_scales), dim=0)
                match_det_depth = torch.cat(
                    (match_det_depth, ocr_match_det_depth), dim=0)
            ...
            for track_id in all_track_ids:
                if track_id not in match_det_ids:
                    ...
                    self.tracks[track_id].obs_scales.append(None)
            ...
            scales = torch.cat((match_det_scales, unmatch_det_scales), dim=0)
            depth = torch.cat((match_det_depth, unmatch_det_depth), dim=0)
