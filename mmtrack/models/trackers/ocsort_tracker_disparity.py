...
class OCSORTTracker_Disparity(KalmanTrackerBase):
    """Tracker for OC-SORT.
    ...
    def ocm_assign_ids(self,
        ...
        # compute distance
        ious = bbox_overlaps(track_bboxes + offset.to(det_bboxes), det_bboxes[:, :4])
    ...
    def ocr_assign_ids(self,
        ...
        ious = bbox_overlaps(track_obs[:, :4] + offset, det_bboxes[:, :4])
        # if offset.sum() > 0:
        #     print(f'\noffset: {offset}')
        #     print('ocr_iou: ', ious, '\nocr_iou2: ', ious2)
        if weight_iou_with_det_scores:
    ...
    def track(self,
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            if len(unmatched_track_inds) > 0:
                # Camera motion is compensated on the persistent Kalman state
                # by Mesh-Affine CMAC (see `estimate_camera_motion`), so the
                # observation-centric recovery below needs no extra offset.
                offset = torch.zeros(4, device=bboxes.device)
