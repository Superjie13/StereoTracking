...
class InterpolateTracklets_Kitti(InterpolateTracklets):
    ...
    def __init__(self,
                 ...
                 use_gsi: bool = False,
                 smooth_tau: int = 10):
        super().__init__(min_num_frames=min_num_frames,
    ...
    def _interpolate_track(self,
        ...
        Args:
            track (ndarray): With shape (N, 18). Each row denotes
                (frame_id, track_id, category, truncated(-1), occluded(-1), alpha(-1),
                 x1, y1, x2, y2, dimension_h(-1), dimension_w(-1), dimension_l(-1),
                 location_x(-1), location_y(-1), location_z(-1), rotation_y(-1), score).
            max_num_frames (int, optional): The maximum disconnected length in
                the track. Defaults to 20.

        Returns:
            ndarray: The interpolated track with shape (N, 18). Each row denotes
                (frame_id, track_id, category, truncated(-1), occluded(-1), alpha(-1),
                 x1, y1, x2, y2, dimension_h(-1), dimension_w(-1), dimension_l(-1),
                 location_x(-1), location_y(-1), location_z(-1), rotation_y(-1), score).
        """
        ...
        frame_ids = track[:, 0]
        interpolated_track = np.zeros((0, 18))
        ...
        for i in np.where(np.diff(frame_ids) > 1)[0]:
            ...
            if 1 < num_disconnected_frames < max_num_frames:
                left_bbox = track[i, 6:10]
                right_bbox = track[i + 1, 6:10]
                ...
                for j in range(1, num_disconnected_frames):
                    ...
                    cur_result = -np.ones((18, ))
                    cur_result[0] = j + left_frame_id
                    cur_result[1] = track_id
                    cur_result[2] = track[i, 2]
                    cur_result[6: 10] = cur_bbox
                    cur_result[17] = track[i, 17]
    ...
    def gaussian_smoothed_interpolation(self,
        ...
        x1 = track[:, 6].reshape(-1, 1)
        y1 = track[:, 7].reshape(-1, 1)
        x2 = track[:, 8].reshape(-1, 1)
        y2 = track[:, 9].reshape(-1, 1)
        gpr.fit(t, x1)
        ...
        gsi_track = [[
            t[i, 0], track[i, 1], track[i, 2], track[i, 3], track[i, 4], track[i, 5],
            x1_gpr[i], y1_gpr[i], x2_gpr[i], y2_gpr[i], track[i, 10], track[i, 11],
            track[i, 12], track[i, 13], track[i, 14], track[i, 15], track[i, 16], track[i, 17]
