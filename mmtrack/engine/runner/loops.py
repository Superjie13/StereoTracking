...
from mmtrack.registry import LOOPS
...
class TeacherStudentValLoop(ValLoop):
    ...
    def run(self):
        ...
        if is_model_wrapper(model):
            model = model.module
        assert hasattr(model, 'adaptor')

        predict_on = model.adaptor.da_test_cfg.get('predict_on', None)
        multi_metrics = dict()
        for _predict_on in ['teacher', 'student']:
            model.adaptor.da_test_cfg['predict_on'] = _predict_on
        ...
        model.adaptor.da_test_cfg['predict_on'] = predict_on
